...
`pid-facilities-management-azure.zip` is ready for Azure App Service Linux. It deliberately excludes `.env` and `data/`, so no local password or records are uploaded.
...
3. Enter the administrator password when prompted. The script creates a Linux B1 App Service plan, configures HTTPS, stores secrets as App Service settings, deploys the ZIP, and prints the live URL.

The server saves data under `/home/pid-facilities-data`, outside the read-only ZIP package. Keep the plan at one instance: the included JSON datastore is suitable for a small team, not multi-instance scaling. For a larger deployment, migrate storage to Azure Database for PostgreSQL or Cosmos DB before scaling out.
